...
class AttentionDecoder(nn.Module):
    def __init__(self, batch_size, hidden_size, emb_size, output_size, n_layers, dropout, device):
        ...
        self.tan_weight = nn.Parameter(torch.zeros(self.batch_size, hidden_size, 1).to(self.device))
        self.dropout = nn.Dropout(dropout)
        self.lstm = nn.LSTM(hidden_size + emb_size, hidden_size, n_layers, batch_first=True) if n_layers==1 else \
            nn.LSTM(hidden_size + emb_size, hidden_size, n_layers, batch_first=True, dropout=dropout)
        self.out = nn.Linear(hidden_size, output_size)

    def forward(self, input, hidden, cell, encoder_outputs):
        embedded = self.dropout(self.embedded(input)) # embedded.size() = [batch_size, 1, emb_size]

        e_out_transposed = encoder_outputs.transpose(0, 1)

        # hidden.size() = [n_layers, batch_size, hidden_size]
        # tan = torch.tanh(self.hid_lin(hidden.transpose(0,1))+self.enc_lin(e_out_transposed)) # tan.size() = [batch_size, max_in, hidden_size]
        tan = torch.mul(self.hid_lin(hidden.transpose(0, 1)), self.enc_lin(
            e_out_transposed))  # tan.size() = [batch_size, max_in, hidden_size]
        ...
        # e_out_transposed.size() = [batch_size, max_length*28, hidden_size]
